...
#@onready var Corridor = preload("res://scripts/Corridor.gd")
#@onready var SimpleCorridor = preload("res://scripts/SimpleCorridor.gd")
...
func build_level():
	...
	UP_ROOM = all_points[random.randi_range(0, len(all_points)-1)]
	all_points.erase(UP_ROOM)
	DOWN_ROOM = all_points[random.randi_range(0, len(all_points)-1)]
	...
	for ring in rooms:
		...
		for t in range(len(ring) - 1, 0, -1):
			var new_c = preload("res://scripts/Corridor.gd").new(
				ring[t],
				ring[t - 1],
				CORRIDOR_THICKNESS,
				[],
...
func find_center_ring_points(
	...
	for x in range(room_count):
		...
		# random
		leg_x += random.randi_range(-point_shift_limit, point_shift_limit)
		leg_y += random.randi_range(-point_shift_limit, point_shift_limit)
...
func place_room(ring: Array, coordinates: Array):
	...
	var size_x = random.randi_range(MIN_ROOM_SIZE, MAX_ROOM_SIZE)
	var size_y = random.randi_range(MIN_ROOM_SIZE, MAX_ROOM_SIZE)
	var a
	...
	ring.append(
		preload("res://scripts/Room.gd").new(coordinates, size_x, size_y, a)
	)
